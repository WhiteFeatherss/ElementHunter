...
FName AIrvalPawnC::MovementComponentName(TEXT("MovementComponent0"));
FName AIrvalPawnC::CollisionComponentName(TEXT("CollisionComponent0"));
FName AIrvalPawnC::SkeletalComponentName(TEXT("SkeletalMeshComponent0"));
FName AIrvalPawnC::MeshComponentName(TEXT("MeshComponent0"));


AIrvalPawnC::AIrvalPawnC(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	CollisionComponent = CreateDefaultSubobject<USphereComponent>(AIrvalPawnC::CollisionComponentName);
	...
	MovementComponent = CreateDefaultSubobject<UPawnMovementComponent, UFloatingPawnMovement>(AIrvalPawnC::MovementComponentName);
	...
	MeshComponent = CreateOptionalDefaultSubobject<UStaticMeshComponent>(AIrvalPawnC::MeshComponentName);
	if (MeshComponent)
	{
	...
	SkeletalMeshComponent = CreateOptionalDefaultSubobject<USkeletalMeshComponent>(AIrvalPawnC::SkeletalComponentName);
	if (SkeletalMeshComponent)
	{
		...
		const float Scale = CollisionComponent->GetUnscaledSphereRadius() / 160.f; // @TODO: hardcoding known size of EngineMeshes.Sphere. Should use a unit sphere instead.
		SkeletalMeshComponent->SetRelativeScale3D(FVector(Scale));
...
void AIrvalPawnC::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (bAddDefaultMovementBindings)
	{
		...
		PlayerInputComponent->BindAxis("DefaultPawn_MoveForward", this, &AIrvalPawnC::MoveForward);
		PlayerInputComponent->BindAxis("DefaultPawn_MoveRight", this, &AIrvalPawnC::MoveRight);
		PlayerInputComponent->BindAxis("DefaultPawn_MoveUp", this, &AIrvalPawnC::MoveUp_World);
		PlayerInputComponent->BindAxis("DefaultPawn_Turn", this, &AIrvalPawnC::AddControllerYawInput);
		PlayerInputComponent->BindAxis("DefaultPawn_TurnRate", this, &AIrvalPawnC::TurnAtRate);
		PlayerInputComponent->BindAxis("DefaultPawn_LookUp", this, &AIrvalPawnC::AddControllerPitchInput);
		PlayerInputComponent->BindAxis("DefaultPawn_LookUpRate", this, &AIrvalPawnC::LookUpAtRate);
	}
}

void AIrvalPawnC::UpdateNavigationRelevance()
{
...
void AIrvalPawnC::MoveRight(float Val)
{
	if (Val != 0.f)
	{
		if (Controller)
...
void AIrvalPawnC::MoveForward(float Val)
{
	if (Val != 0.f)
	{
		if (Controller)
...
void AIrvalPawnC::MoveUp_World(float Val)
{
	if (Val != 0.f)
	{
...
void AIrvalPawnC::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds() * CustomTimeDilation);
}

void AIrvalPawnC::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds() * CustomTimeDilation);
}

UPawnMovementComponent* AIrvalPawnC::GetMovementComponent() const
{
